...
class HireDeveloperFinalProposalController extends Controller
{
        ...
        try {
            ...
            if (!($userData['exist'] == 1 && $userData['privileges'] == 1 && $userData['type'] == 1 && $userData['verified'] == 1)) {
                ...
                if ($proposal === null) {
                    ...
                    $response = Controller::returnResponse(200, "successful", ["resources" => $rr]);
                    return (json_encode($response));
    ...
    function getContractData($proposalIds, $teamId = 0, $page = 0, $limit = 0)
    {
        ...
        if ($teamId > 0) {
            ...
            $contracts = hire_developer_final_proposal::whereIn('proposal_id', $proposalIds)->where('status', '>', -1)->select('*')->latest()->offset($page)->limit($limit)->get();
