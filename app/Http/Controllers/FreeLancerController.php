    ...
    function get_freelancer_info($id)
    {
        try {
            $user = DB::table('users')
                ...
                ->where('users.id', $id)
                ->select('users.id','users.first_name','users.last_name','users.email','users.role','users.dob','users.gender','users.type','users.token','freelancers.type_freelancer','freelancers.bio','freelancers.country','freelancers.image','freelancers.tools',)
                ->get();
